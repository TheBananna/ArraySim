...
void add_equitri_array(Array* arr, vec3d center, vec3d dir, vec2i dims, double spacing)
{
	...
	double zspacing = spacing * sin(M_PI / 3);//vertical spacing is different due to equalateral triangle setup
	int row = 0;
	//goes left to right top to bottom
	for (double z = zspacing * (dims(0) - 1) / 2.; z >= -zspacing * (dims(1) - 1) / 2.; z -= zspacing)
	{
		for (double x = -spacing * (dims(1) - 1 - row % 2) / 2.; x <= spacing * (dims(1) - 1 - row % 2) / 2.; x+=spacing)
		{
			vec3d el = { x, 0, z };
			el = rot_mat * el;
			el += center;
